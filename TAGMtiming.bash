...
if [[ $# > 0 ]]; then
    line=`expr $1 + 1`
    run=`head -n $line TAGMtiming_runs.list | tail -n 1`
...
echo $HALLD_HOME/$BMS_OSNAME/bin/hd_root -PJANA:BATCH_MODE=1 -PPRINT_PLUGIN_PATHS=1 -PPLUGINS=TAGM_timing -PTHREAD_TIMEOUT_FIRST_EVENT=3000 -PTHREAD_TIMEOUT=3000 -PEVIO:RUN_NUMBER=$run $infiles
$HALLD_HOME/$BMS_OSNAME/bin/hd_root \
  -PPRINT_PLUGIN_PATHS=1 \
  -PJANA:BATCH_MODE=$batch \
  -PPLUGINS=TAGM_timing \
...
if [[ $retcode = 0 ]]; then
    mv hd_root.root TAGMtiming_${run}.root
fi
exit $retcode
